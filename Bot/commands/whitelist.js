...
module.exports = {
    ...
    let prefix = db.get(`${message.guild.id}_PREFIX`) || require("../config/config.json").prefix
	  ...
	  if (!args[1] || !args[2] || !["add","remove"].includes(args[1].toLowerCase())) return message.reply(`**❌ contoh: \`${prefix}whitelist [add/remove] [user]\`**`)
   ...
   if (args[1] == "add")  {
    ...
    message.reply({content: `✅ **sukses menambahkan ${user} kedalam Whitelist**`})
 ...
 const hh = db.get(`${message.guild.id}_PROTECTIONLOG`)
	...
	Mo7amd.send({embeds: [embed]}).catch(() => {})  
     ...
     if (!db1.get({key: `${message.guild.id}_WHITELISTS`})) {
		...
		if (!db1.get({key: `${message.guild.id}_WHITELISTS`}).includes(user.id)) return message.reply({content: `**❌ Pengguna Ini Tidak Ada Dalam Whitelist**`, ephemeral: true})
    ...
    message.reply({content: `**✅ Berhasil menghapus ${user} dari Whitelist**`})
		
		let embed = new Discord.MessageEmbed()
.setTitle(`Whitelist Baru Dihapus`)
.setThumbnail(message.guild.iconURL({dynamic: true }))
.setAuthor(`${message.author.tag}`,`${message.author.displayAvatarURL({ dynamic: true })}`)  
.setColor("BLUE")
