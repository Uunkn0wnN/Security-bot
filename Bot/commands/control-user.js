...
module.exports = {
   ...
   if (args[1] == "add")  {
		...
		if (db1.get({key: `${message.guild.id}_CONTROLUSERS`}).includes(user.id)) return message.reply({content: `**❌Pengguna Ini Sudah Di Control Users**`})
    ...
    message.reply({content: `✅ **sukses menambahkan ${user} ke Control Users**`})
		let embed = new Discord.MessageEmbed()
.setTitle(`Control User baru ditambahkan`)
 ...
 const hh = db.get(`${message.guild.id}_PROTECTIONLOG`)
	...
	Mo7amd.send({embeds: [embed]})  .catch(() => {})  
     ...
     if (!db1.get({key: `${message.guild.id}_CONTROLUSERS`})) {
		...
		if (!db1.get({key: `${message.guild.id}_CONTROLUSERS`}).includes(user.id)) return message.reply({content: `**❌ Pengguna ini Tidak Ada di Control Users**`})
    ...
    message.reply({content: `**✅ sukses menghapus ${user} dari Control Users**`})
