...
module.exports = {
    ...
    execute (message, client) {
			...
			if (!args[1]) return message.reply("**❌ tag role Anda ingin mencari anggota di dalamnya**")
			...
			if (!role) return message.reply("**❌ tidak bisa mencari role ini**")
			if (!message.member.permissions.has("MANAGE_ROLES")) return message.reply({content: `**anda tidak mempunyai permission : \`MANAGE_ROLES\`**`, ephemeral: true})
