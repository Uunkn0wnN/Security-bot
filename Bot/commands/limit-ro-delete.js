...
module.exports = {
   ...
   if (!limit) return message.reply("**❌ Ketik nomor untuk menetapkan limit**")
...
.setTitle(`Limit telah di Update`)
.setThumbnail(message.guild.iconURL({dynamic: true }))
.setAuthor(`${message.author.tag}`,`${message.author.displayAvatarURL({ dynamic: true })}`)  
.setColor("BLUE")
.setDescription(`**By : ${message.author}\nAction : Update Limit Roles Delete\Limit baru : ${limit}\nDate On : ${require("moment")(new Date()).format("DD/MM/YYYY hh:mm")}**`)
.setFooter(message.guild.name, message.guild.iconURL({dynamic:true}))
  
  message.reply({content: `**✅ sukses update roles hapus limit ke \`${limit}\`**`})
