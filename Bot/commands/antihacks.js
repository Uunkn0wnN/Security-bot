...
module.exports = {
	...
	execute(message, client) {
	 ...
	 if (!db1.get({key: `${message.guild.id}_CONTROLUSERS`}).includes(message.author.id) && message.author.id !== message.guild.ownerId) return message.reply({content: '**❌ hanya satrio yang dapat menggunakan nya**'})
      ...
      .setDescription(`> **Action : **nyalakan Antihacks System
> **By :** ${message.author}
> **Date :** ${require("moment")(new Date()).format("DD/MM/YYYY hh:mm")}`)
      ...
      .setTimestamp()
			...
			message.reply({content: `**✅ Antihacks telah di nyalakan**`})
       ...
       let embed = new Discord.MessageEmbed()
			.setAuthor(message.author.tag, message.author.avatarURL())
      .setThumbnail(message.guild.iconURL())
      .setDescription(`> **Action : **matikan Antihacks System
> **By :** ${message.author}
> **Date :** ${require("moment")(new Date()).format("DD/MM/YYYY hh:mm")}`)
			.setColor("RED")
      .setFooter(message.guild.name, message.guild.iconURL())
      .setTimestamp()
			...
			message.reply({content: `**✅ Antihacks telah di matikan**`}) 
